...
def main() -> None:
    ...
    for ax, profile in zip(axes, profiles):
        ...
        if part.empty:
            continue
        summary = summarize_with_iqr(part, "expanded_nodes", ["experiment", "n_attractions"])
        ...
        ax.grid(alpha=0.2)
    axes[0].set_ylabel("expanded_nodes (median, IQR)")
    axes[-1].legend()
    fig.suptitle("Expanded nodes scaling (fixture, canonical suites)")
