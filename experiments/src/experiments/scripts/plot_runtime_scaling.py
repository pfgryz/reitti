...
def _canonical_runtime_subset(df):
...
def main() -> None:
    ...
    base = _canonical_runtime_subset(base)
    if base.empty:
        return
    ...
    for ax, profile in zip(axes, profiles):
        ...
        if part.empty:
            continue
        summary = summarize_with_iqr(part, "wall_time_ms", ["experiment", "n_attractions"])
    ...
    axes[-1].legend()
    fig.suptitle("Runtime scaling (fixture, canonical suites)")
